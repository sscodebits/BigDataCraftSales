 ...
 *
 */
public class SalesAnalyzerSimple implements SalesAnalyzer, Serializable {
	...
	public void process(JavaSparkContext sc, JavaRDD<String> customers, 
		...
		//// check matches rows against parser
		//JavaRDD<AccessLogRow> unmatched = rows.filter(row -> !row.is_bValid());
		//JavaRDD<AccessLogRow> matched = rows.filter(row -> row.is_bValid());
		
		//System.out.println("******** CUSTOMER ROWs " + customerRows.count());

		/*
		// store the rows in Output Dir
		JavaPairRDD<String, Integer> output = customerRows
				.mapToPair(customer -> new Tuple2<>(customer.getState(), 1))
				.reduceByKey((a,b) -> a+b);
				*/
